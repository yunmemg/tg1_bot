...
async def cmd_fav(message: Message):
    ...
    for idx, row in enumerate(rows, start=1):
        lines.append(
            f"{idx}. [{PLATFORM_NAMES.get(row['platform'], row['platform'])}] "
            f"{row['title']} - {row['artist']}"
        )
        kb_buttons.append(
            [
                InlineKeyboardButton(text=f"▶️ {idx}. {row['title']}", callback_data=f"playfav:{row['id']}"),
...
async def cb_delfav(callback: CallbackQuery):
    ...
    if ok:
        await callback.message.edit_text(f"已从歌单中删除该收藏。")
    else:
